...
def _greedy_split(cards, order, split_bomb):
    """
    单趟贪心拆解。
    order: [三张优先位, 连对优先位, 顺子优先位]
    split_bomb: 是否拆炸弹
    返回手数（int）
    """
    freq = count_ranks(cards)
    ...
    if split_bomb:
        # 拆炸弹：不计手数，4张当普通牌参与后续拆解
        pass
    else:
        # 不拆炸弹：炸弹计1手并消耗
        for r in bomb_ranks:
            freq[r] = 0
        hands += len(bomb_ranks)

    # 3. 按 order 顺序处理三张/连对/顺子
    step_count = [0, 0, 0]  # [三张组数, 连对组数, 顺子组数]
    ...
    hands += step_count[0] + step_count[1] + step_count[2]
...
def detect_structure_loss(rem_freq):
    """结构损失系数。返回浮点数。"""
    ranks = sorted(r for r in rem_freq if rem_freq[r] > 0)
    if not ranks:
        return 1.2
    ...
    # 检查连对
    has_pair_run = False
    pair_ranks = [r for r in ranks if rem_freq[r] >= 2]
    i = 0
    while i < len(pair_ranks):
        j = i
        while j + 1 < len(pair_ranks) and pair_ranks[j + 1] == pair_ranks[j] + 1:
            j += 1
        if j - i + 1 >= 3:
            has_pair_run = True
            break
    ...
    # 检查飞机带翅膀
    triple_ranks = [r for r in ranks if rem_freq[r] >= 3]
    can_carry = False
    single_count = sum(1 for r in ranks if rem_freq[r] == 1)
    pair_count = sum(1 for r in ranks if rem_freq[r] == 2)
    if triple_ranks and (single_count > 0 or pair_count > 0):
        can_carry = True
